    ...
    <div class="subs-container" v-if="response==false">
      ...
      <div class="subs-container" style="background-color: #2D2D2D;height: 167px;">
        <div style="margin: 2% 30%;">
          <img src="/src/assets/failure-block.svg" style="width:56%;margin:9%;margin-top: 0;"></img>
          <h4 v-if=" altMessages && altMessages.messageTransactionFailed">{{altMessages.messageTransactionFailed}}</h4>
        </div>
      </div>
  <!-- NOTE function process changes reviewed -->
      <div class="subs-plan">
  ...
  export default {
    ...
    methods: {
        ...
        homeRedirect() {
            ...
            // If user comes back to whohoo page after subscription,
            // When he clicks on the start watching button
            // User should be taken to the content page
            // (Scenario) Page will refresh after the subscription
            // Coded by Sumit (05-01-18)
      ...
      initPaymentResult() {
        console.log("initPaymentResult || reached", localStorage.getItem("redirect_campaign"));
         if (localStorage.getItem("redirect_campaign")) {
              
                window.location.replace(location.protocol + '//' + location.host + "/paymentStatus");
                return;
            }
        ...
        this.actProductResponse().then((response) => {
          ...
          setTimeout(() => {
            ...
            if (window.mobileAnalyticsClient) {
              ...
              monetizEventAttributeObj.monetization_transaction_store = "Payu";
      ...
      processOrders(orders) {
        ...
        if (tvodPlans && tvodPlans.length) {
          ...
          for (var i in tvodPlans) {
            var timeDiff = (new Date().getTime() - new Date(tvodPlans[i].dates.valid_to).getTime()) / (1000 * 3600 * 24);
            if (timeDiff < 0) {
        ...
        if (activeSubsOrders && activeSubsOrders.length) {
          ...
          for (var i in activeSubsOrders) {
            var timeDiff = (new Date().getTime() - new Date(activeSubsOrders[i].dates.valid_to).getTime()) / (1000 * 3600 * 24);
            if (timeDiff < 0) {
              hasActivePlan = true;
              activePlanIndex = i;
              break;
